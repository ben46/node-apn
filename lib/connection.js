...
Connection.prototype.connect = function () {
	...
	this.initialize().then(function () {
		...
		this.socket.on("error", function(error) { self.errorOccurred(this, error); });
		this.socket.on("timeout", function () { self.socketTimeout(this); });
		this.socket.on("data", function (data) { self.handleTransmissionError(this, data); });
		this.socket.on("drain", function() { self.socketDrained(this); });
		this.socket.on("clientError", function () { self.errorOccurred(this); });
		this.socket.once("close", function () { self.socketClosed(this); });
...
Connection.prototype.errorOccurred = function(socket, err) {
	...
	if(this.deferredConnection && !this.deferredConnection.promise.isResolved()) {
...
Connection.prototype.socketAvailable = function(socket) {
	//if (!socket || (socket.socket.bufferSize !== 0 || !socket.writable)) {
 ...
 * @private
 */
Connection.prototype.socketDrained = function(socket, dontServiceBuffer) {
	...
	if(dontServiceBuffer === true) {
		process.nextTick(function () { this.serviceBuffer(); }.bind(this));
	}
};
...
Connection.prototype.socketClosed = function(socket) {
	...
	if (socket === this.socket && !this.deferredConnection.promise.isResolved()) {
	...
	if (data[0] == 8) {
		...
		if (foundNotification) {
			...
			if(socket.cachedNotifications.length > 0) {
				var differentialSize = socket.cachedNotifications[0]['_uid'] - notification['_uid'];
...
Connection.prototype.raiseError = function(errorCode, notification) {
	...
	if (notification && typeof notification.notification.errorCallback == 'function' ) {
		notification.notification.errorCallback(errorCode);
...
Connection.prototype.transmitNotification = function(socket, notification) {
	...
	if (messageLength > 255) {
		process.nextTick(function () {
			...
			this.emit('transmissionError', Errors['invalidPayloadSize'], notification);
		}.bind(this));
		return false;
	}
 ...
 * @param {String|Buffer|String[]|Buffer[]} recipient The token(s) for devices the notification should be delivered to.
 * @since v1.3.0
 */
Connection.prototype.pushNotification = function (notification, recipient) {
	if (sysu.isArray(recipient)) {
		...
		if (typeof recipient == "string") {
			recipient = new Buffer(recipient.replace(/\s/g, ""), "hex");
		}
...
Connection.prototype.sendNotification = function (notification) {
	...
	this.pushNotification(notification, token);
	return 0;
};
